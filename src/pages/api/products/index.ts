...
const handler = (request: NextApiRequest, response: NextApiResponse<ProductsResponse | ErrorResponse>): void => {
  const { method, query } = request;
  const { status } = response;

  switch (method) {
    case "GET": {
      const stringifiedPage = Array.isArray(query.page) ? query.page.join("") : query.page;
