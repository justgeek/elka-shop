...
const handler = (request: NextApiRequest, response: NextApiResponse<ProductResponse | ErrorResponse>): void => {
  const { method, query } = request;
  const { status } = response;

  switch (method) {
    case "GET": {
      const stringifiedGtinQuery = Array.isArray(query.gtin) ? query.gtin.join("") : query.gtin;
